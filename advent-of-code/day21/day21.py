...
input_file = os.path.join(os.path.dirname(__file__), "day21.txt")
...
# Define the keypad and arrows
keypad = [list('789'), list('456'), list('123'), list(' 0A')]
arrows = [list(' ^A'), list('<v>')]

# Create dictionaries for keypad and arrows for quick lookup
keypad_dict = {}
...
# Function to calculate keystrokes on arrows keypad
@cache
...
# Function for recursive keystroke calculation on arrows keypad
@cache
...
# Function for keystroke calculation with recursive calls on keypad
@cache
...
# Function for keystroke calculation with recursive calls (more levels)
@cache
...
# Time-profiled solution for part 1
@profiler
def part1(arg):
    calculate = 0
    for line in arg:
